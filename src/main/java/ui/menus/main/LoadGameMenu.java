...
public class LoadGameMenu {
    ...
    public static void show(MultiWindowTextGUI gui, BasicWindow window)
    {
        ...
        SelectableActionListBox savesMenu = new SelectableActionListBox();
        ...
        for (int i = 0; i < 6; i++) {
            if (i < saves.length) {
                ...
                savesMenu.addItem(saveName, () -> {
                    Player player = PlayerRepository.load(saveName);
                    if (player != null) {
                        Area area = AreaFactory.create(player.getCurrentArea());

                        AreaManager.enter(area, gui, window, player);
                    }
                }
                );
            }else {
                int slot = i + 1;
                savesMenu.addItem("Slot " + slot + " - Empty", () -> {
                    System.out.println("Slot vazio");
                });
            }
        }

        savesMenu.setLayoutData(LinearLayout.createLayoutData(LinearLayout.Alignment.Center));
        ...
        Button back = new Button("Back", () -> MainMenu.show(gui, window));

        back.setLayoutData(LinearLayout.createLayoutData(LinearLayout.Alignment.Center));
